...
exports.forgotPassword = catchAsyncErrors ( async( req, res, next) =>{
    ...
    if (!user){
        return next(new ErrorHandler(`Usuario (${req.body.email}) no se encuentra registrado`, 404))
    }
    ...
    try{
        await sendEmail({
            email:user.email,
            subject: "Esencial Home Recuperación de la contraseña",
            mensaje
        })
        res.status(200).json({
            success:true,
...
exports.updateProfile= catchAsyncErrors(async(req,res,next)=>{
    //Actualizar el email desde usuario
    const newUserData ={
